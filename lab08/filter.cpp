...
extern "C" void grayFP(char* pixelsStart, int numPixels);
extern "C" void darkenSIMD(char* pixelsStart, int numPixels);

int main(int argc, char* argv[]) {
  if (argc != 4 || (string(argv[1]) != "gray" && string(argv[1]) != "darken")) {
    cout << "Usage: " << argv[0] << " op input.bmp output.bmp\n";
    cout << "  where op is either \"gray\" or \"darken\"\n";
    return 1;
  }
  ...
  for (int row = 1; row <= height; row++) {
    ...
    if (string(argv[1]) == "gray")
      grayFP(rowStart, numberOfPixelsToProcess);
    else
      darkenSIMD(rowStart, numberOfPixelsToProcess);
